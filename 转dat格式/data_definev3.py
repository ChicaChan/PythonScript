...
class DataProcessor:
    ...
    def save_files(self) -> None:
        ...
        # 后处理define文件
        self.post_process_define(define_path)
    ...
    def post_process_define(self, define_path: str) -> None:
        import re
        from collections import defaultdict
            ...
            for line in f:
                ...
                if line.endswith(';') and ';' in line:
                    ...
                    if len(parts) >=2 and parts[0] and parts[1].isdigit():
                        base_max_sub[parts[0]] = int(parts[1])
        ...
        pattern = re.compile(r'^(di|dw|dc)\s+\$([A-Za-z]+\d+)(0)(\d+)=\$.*$')
        ...
        for idx, line in enumerate(lines):
            ...
            if match:
                ...
                sub_num = int(match.group(4))
                if base_part in base_max_sub:
                    base_groups[base_part].append((idx, sub_num, line))

        # 重新编号处理
        for base, max_sub in base_max_sub.items():
            if base not in base_groups:
                continue
            
            # 按原始子题号排序
            entries = sorted(base_groups[base], key=lambda x: x[1])
            
            # 生成新编号
            for order, (idx, _, original_line) in enumerate(entries, start=1):
                if order > max_sub:
                    break
                
                # 保持数字位数不变
                width = len(match.group(4)) if (match := pattern.match(original_line.strip())) else 0
                new_sub = f"{order:0{width}d}"
                
                # 替换序号部分
                new_line = re.sub(
                    r'(\$' + re.escape(base) + r'0)(\d+)',
                    rf'\g<1>{new_sub}',
                    original_line.strip()
                ) + '\n'
                lines[idx] = new_line
