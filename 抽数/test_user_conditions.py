...
for i, test in enumerate(test_cases, 1):
    ...
    # 验证转换后的条件是否可以在pandas中使用
    if converted != 'total':
        try:
            # 尝试在测试数据框上评估条件
            test_df.eval(converted)
            print(f'   验证结果: ✓ (语法正确)')
            successful_conversions += 1
        except Exception as e:
            print(f'   验证结果: ✗ (语法错误: {str(e)})')
            failed_conversions += 1
            failed_cases.append((test, converted, str(e)))
    else:
        print(f'   验证结果: ✓ (total条件)')
        successful_conversions += 1
    
    print('-' * 80)

print(f'\n测试总结:')
print(f'总条件数: {len(test_cases)}')
print(f'成功转换: {successful_conversions}')
print(f'失败转换: {failed_conversions}')

if failed_conversions > 0:
    print('\n失败的条件:')
    for i, (original, converted, error) in enumerate(failed_cases, 1):
        print(f'{i}. 原始条件: {original}')
        print(f'   转换结果: {converted}')
        print(f'   错误信息: {error}')
        print('-' * 50)
else:
